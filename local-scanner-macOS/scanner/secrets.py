...
MAX_FINDINGS   = 5000          # bail out after this many to keep UI snappy
# Per-pattern cap.  The generic env-assignment regex used to dominate the
# total findings budget on normal codebases - we now cap each rule on its own
# so one chatty rule can't exhaust the global ceiling.
PER_PATTERN_MAX = 200
# Substrings that almost always mean "obvious placeholder / env var reference",
# not a real secret.  When the matched value contains one of these we skip it.
PLACEHOLDER_HINTS = (
    "your_", "example", "changeme", "placeholder", "redacted",
    ...
    for path in _iter_text_files(root):
        if len(findings) >= MAX_FINDINGS:
            findings.append({
                "category": "secrets",
                "id": "truncated",
                "title": f"Stopped after {MAX_FINDINGS} findings (scan was bigger than the cap)",
                "severity": "HIGH",
                "evidence": f"Last file inspected: {path} (~{files_scanned} files scanned)",
        ...
        for fid, severity, pattern, description in PATTERNS:
            if per_pattern_counts.get(fid, 0) >= PER_PATTERN_MAX:
                continue
            for m in pattern.finditer(text):
                ...
                per_pattern_counts[fid] = per_pattern_counts.get(fid, 0) + 1
                if per_pattern_counts[fid] >= PER_PATTERN_MAX:
                    findings.append({
                        "category": "secrets",
                        "id": f"{fid}-capped",
                        "title": f"{description}: stopped reporting after {PER_PATTERN_MAX} matches",
                        "severity": "INFO",
                        "evidence": "",
                        "remediation": "Tighten this pattern in secrets.py or fix the existing matches first",
                    })
                    break
                if len(findings) >= MAX_FINDINGS:
                    break
