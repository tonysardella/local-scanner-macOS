...
While a scan runs you'll see a segmented progress bar (one segment per selected scan, lights up green when that scan finishes) and a live "files scanned" counter for the secrets walk. Long scans (e.g. `~/`) can be cancelled mid-run.
